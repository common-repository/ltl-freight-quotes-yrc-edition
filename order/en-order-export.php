...
if (!class_exists('EnOrderExport')) {
    ...
    class EnOrderExport
    {
        ...
        public function en_orders_sending($orders, $en_orders_ids)
        {
            ...
            $post_data = [
                'serverName' => yrc_quotes_get_domain(),
                'licenseKey' => get_option('wc_settings_yrc_plugin_licence_key'),
                'currencyUnit' => get_option('woocommerce_currency'),
                'platform' => 'wordpress',
                'orders' => $orders,
            ];

            $curl_obj = new YRC_Curl_Request();
            $response = $curl_obj->yrc_get_curl_response($url, $post_data);
